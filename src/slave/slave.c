...
void do_task(char * file_name) {
    ...
    sprintf(command, "md5sum %s | sed -r 's/ .*\\..*/ - File Name:&/g'", file_name);
    FILE * output = popen(command, "r");

    if (output == NULL) {
        fprintf(stderr, "ERROR: could not execute md5 on file %s", file_name);
        exit(1);
    }
...
void wait_more_tasks() {
    ...
    while((dim = read(STDIN_FILENO, buff, MAX_COMMAND_LENGTH)) > 0) {
        buff[dim-1] = 0; // Esto es porque lo hago a mano con la terminal, por lo que tengo que apretar enter y agrega un \n. Posiblemente haya que sacarlo
        do_task(buff);
        memset(buff, 0, dim);
    }
}